...
STANDS = [
    ...
    # ВЖИВЛЕНО 2026-09-05, после того как порог CI отстал ДВАЖДЫ за день.
    # Ставится СЧЁТОМ (`--fix`), а не рукой: число, набираемое пальцами,
    # рано или поздно наберут неверно, а стенд, краснеющий из-за этого на
    # каждом коммите, рано или поздно замолчат. Испытан на падение.
    ("inventory/ПОРОГ-CI.py", ["ПОРОГ CI ЗЕЛЕНО", "расхождений 0"]),
