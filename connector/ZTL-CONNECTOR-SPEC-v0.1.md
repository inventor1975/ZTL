...
a downstream consumer's job or a later stage):** the multi-judge tree, BFT consensus, the
...
- **a downstream consumer** (reference protocol): the warrant-form → signed re-computable
  verdict shape.
...
1. The connector defines **semantics + artifact + verification**, NOT transport,
   packaging, or governance. Those live in the protocol ABOVE (a downstream consumer, or
...
2. A verdict is verifiable **without any protocol's servers** (open reference +
   ...
   reference — **not in a downstream consumer**.
