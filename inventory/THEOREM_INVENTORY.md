...
**Update 2026-09-24 (later):** **1143 theorems across 68 modules** — the new module `ZParabola` (14 theorems): a negative discriminant leaves no root, the case the solver's refusal rests on (`form_pos`: B² < 4AC → B·P·Q < A·P² + C·Q²), with clean `Nat` replacements for core lemmas that carry `propext`; empty axiom list.
...
python3 inventory/axiom_audit.py   # 1143 theorems across 68 modules, empty axiom list
...
## Tier A — machine-proved (1143 theorems, empty axiom list)
...
| the six engineering traditions (NaN, SQL NULL, taint, abstract interpretation, imprecise probability, provenance semirings) **implement fragments of one logic** | §1, Abstract | each of the six is exhibited on a worked case with a stand — NaN and intervals `zarith.py`, NULL `zsets.py`/`zfuncs.py`, taint `zfuncs.py`/`zprob.py`, Dempster–Shafer `zprob.py`, provenance semirings `zcombine.py`. So the correspondences are demonstrated, not asserted. **A worked instance is not an embedding** — and as of 2026-09-05 none of the six rests on one. The evidence is "here is that tradition's central move, reproduced by our core", which is tier C by construction. **Upgraded (2026-08/09):** ALL SIX are embeddings on the empty axiom list — provenance semirings (`ZProv`), Dempster–Shafer (`ZDempster`), abstract interpretation (`ZAbsInt`, one atom, exactness proved and its price named), IEEE NaN (`ZNaN`: predicates as signs, `!=` the one N-sign), SQL NULL (`ZNull`: lazy register inside, the four signs at WHERE/CHECK/IS, two equalities), taint / information flow (`ZFlow`: Denning's join as the common target, three positions on declassification). The row is kept as the record of what tier D looked like; the claim itself is now tier A. |
...
Corpus now (2026-09-24) 1143 theorems in 68 modules, all clean.
