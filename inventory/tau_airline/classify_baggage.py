...
def main():
    ...
    print(f"  ZTL-SETTLES НАБЛЮДЕНО                    : {len(settles)}")
    print(f"  ПЕРЕКРЫТИЕ С БАЗИСОМ                     : {gen_set}")
    print(f"  ОСТАТОК ЗА ПРЕДЕЛАМИ ИСПЫТАННОГО БАЗИСА  : {residual}")
    print("  («уникально наш» СНЯТО: другой решатель, метод ценности")
    print("   информации или специально построенный отборщик могут дать")
    print("   то же. Остаток говорит только про ЭТОТ базис.)")
    print(f"\n  ОТДЕЛЬНАЯ ТАБЛИЦА, не тот же знаменатель:")
    print(f"    СОКРАЩЕНИЙ берёт базис: {gen_short} из {len(short_circuit)}"
          f" — это свойство БАЗИСА, а не перекрытие с settles")
    if residual == 0 and len(settles) == 0:
        print("\n  ЗНАМЕНАТЕЛЬ ПЕРЕКРЫТИЯ РАВЕН НУЛЮ: настоящего settles не наблюдалось,")
        print("  значит про остаток нельзя утверждать ничего.")
