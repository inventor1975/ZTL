...
Текст, код и доказательства созданы в соавторстве с ИИ-системой Claude
(Anthropic) в режиме диалога; все проектные решения и выбор развилок —
за автором-человеком. Все численные утверждения проверяемы кодом
репозитория (`run_all.py` — полная регрессия), все Lean-утверждения —
ядром Lean 4 (пустой список аксиом, `#print axioms`).
