...
def main() -> int:
    ...
    # ФОРК БОЛЬШЕ НЕ РЕЖЕТ СТОР — слово куратора 2026-08-30, дословно:
    # «Атом стор не разбиваем форком отныне. Делаем разбивки абзацами до
    # определённой длины конкатенируя их».
    #
    # Раньше умолчанием была форковая атомизация, а `--direct` — исключением.
    # Теперь наоборот, и это СПИСОК РАЗРЕШЕНИЙ, а не запретов: форковый путь
    # никуда не делся, но его надо позвать вслух флагом `--fork`. Умолчание,
    # которое тратит деньги, — плохое умолчание.
    #
    # И довод не только про цену. Промерено 2026-08-26: атомизатор МОЛЧА
    # ТЕРЯЕТ — 188 афоризмов из 705 ушли порогом длины, 89% атомов британского
    # акта оказались меню сайта. Сырой абзац не теряет ничего: он и есть текст.
    pa.add_argument("--fork", action="store_true",
                    help="СТАРЫЙ путь: раздать документы форкам на извлечение "
                         "утверждений. Тратит токены куратора и МОЛЧА теряет "
                         "(промерено). Умолчание — машинная нарезка абзацами.")
    pa.add_argument("--direct", action="store_true",
                    help="(умолчание с 2026-08-30) БЕЗ форков: абзац = единица, "
                         "сразу в стор. Оставлено ради старых команд.")
    ...
    # ДОБОР ДО 2000 — умолчание с 2026-08-30, по слову куратора «конкатенируя
    # их». Порог не с потолка: промерено 2026-08-26, медиана поста блога 195
    # знаков, 95% короче 751, длиннее двух тысяч всего 20 из 841.
    # ВНИМАНИЕ: для САМОСТОЯТЕЛЬНЫХ единиц (афоризмы, теоремы, клаузы права)
    # добор ВРЕДЕН — он делает кашу и ломает адресность. Там ставить 0.
    pa.add_argument("--min-chars", type=int, default=2000,
                    help="добирать подряд идущие абзацы до N знаков (умолч. 2000).\n"
                         "0 = не сливать — для афоризмов, теорем, клауз права,\n"
                         "где единица самостоятельна.\n"
    ...
    a = ap.parse_args()
    if a.cmd == "atomize":
        # УМОЛЧАНИЕ — МАШИНА. Форковые пути живут, но их надо позвать `--fork`.
        # Слово куратора 2026-08-30: «атом стор не разбиваем форком отныне».
        if not a.fork:
            if a.batch or a.chunk:
                print("ОТКАЗ: --batch и --chunk имеют смысл только с --fork "
                      "(это разбиение задач ДЛЯ форков). Без --fork стор режется "
                      "машинно абзацами; убери флаг или добавь --fork.",
                      file=sys.stderr)
                return 2
            atomize_direct(a.corpus, pathlib.Path(a.src_root), pathlib.Path(a.store),
                           min_chars=a.min_chars, min_words=a.min_words)
            return 0
        print(f"[ФОРК] позван явно: документы уйдут форкам на извлечение, "
              f"это тратит токены куратора", file=sys.stderr)
        if a.batch:
