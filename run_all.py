...
STANDS = [
    ...
    # the two sides of a numeric comparison read together (2026-09-24)
    ("test_joint_sides.py", ["JOINT SIDES GREEN"]),
    ...
    # ИЗЪЯТО 2026-09-03: inventory/unexecuted_verification.py и
    # inventory/assurance_incident_001.py + его байт-замороженная фикстура.
    # Оба стояли на объектах, которых в этом репозитории больше НЕТ:
    # подписанный тег входа внешнего потребителя (= e819dec7) и blob
    # 6536d14:tool/warrant_receipt.py исчезли при изъятии имени этого
    # потребителя из истории. Имена изъятого здесь НЕ называются — иначе
    # изъятие отменяется тем самым текстом, который его объясняет. Стенды переехали целиком, подлинными байтами, в
    ...
    # ВЖИВЛЕНО 2026-09-05. Ярус надёжности четырёх правил γ/δ — ПРОМЕР перед
    # тем, как начинать порт §27. Три правила на пустом списке живут в
    # корпусе; четвёртое (F:∀ → N, ему нужно ¬∀→∃¬) сломало бы инвариант и
    # потому лежит ВНЕ корпуса, в probes/, и меряется этим стендом.
    # Падает в ОБЕ стороны, испытано: если четвёртое станет чистым — красно
    # (находка), если чистое загрязнится — красно.
    ("inventory/ПАРАМЕТР-ЯРУС.py", ["ЯРУС γ/δ ЗЕЛЕНО", "расхождений 0"]),
    # ПОРОГ-CI.py В НАБОР НЕ ВХОДИТ, И ЭТО ИСПРАВЛЕНИЕ, А НЕ УПУЩЕНИЕ.
    # Я вживил его 2026-09-05 и тут же получил КРАСНОЕ у paper_claims.py:
    # мой стенд гонит СВОЙ `lake build` параллельно с чужими — ровно та
    # гонка, которую шапка ci-sim.sh описывает дословно про этот самый файл.
    # И сторожил он то, что paper_claims уже сторожит («CI floor tracks the
    # corpus»). Два сторожа на одном предмете — не вдвое надёжнее, а
    # источник ложного красного. Прибор остался как ЧИНИЛКА:
    #     python3 inventory/ПОРОГ-CI.py --fix
    # Проверку держит paper_claims.py.
