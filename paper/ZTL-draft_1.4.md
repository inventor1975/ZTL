...
and the frame's own mini-theorems, twenty-seven modules in all — is
formalized in Lean 4 **with an empty axiom list, definitions
included**: 410 theorems, each one audited individually rather than by
...
line reads otherwise. **410 of 410 clean**, re-run by CI on every push.
