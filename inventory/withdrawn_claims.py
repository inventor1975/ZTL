...
WITHDRAWN = [
    ...
    # собой, подделка с пересчётом отпечатка проходит (промерено стендом
    # происшествия, пункт 3; стенд вынесен из этого репозитория 2026-09-03
    # вместе с историей, на которой стоял — см. приписку в
    # inventory/ASSURANCE-INCIDENT-001.manifest). Отзыв ФОРМАЛЬНЫЙ:
