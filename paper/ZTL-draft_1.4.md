...
and the frame's own mini-theorems, thirty-one modules in all — is
formalized in Lean 4 **with an empty axiom list, definitions
included**: 444 theorems, each one audited individually rather than by
...
line reads otherwise. **444 of 444 clean**, re-run by CI on every push.
