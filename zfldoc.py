...
def main():
    ...
    if "--check" in sys.argv:
        ...
        return 0
    # The HTML reference page is a studio artifact; write it only where a
    # static/ directory exists (the ztlstudio repo). In the ZTL repo the
    # language check above is what matters, and it has already passed.
    if os.path.isdir(os.path.dirname(OUT)):
        open(OUT, "w", encoding="utf-8").write(page())
        print(f"  written: {OUT}  ({os.path.getsize(OUT)} bytes)")
    else:
        print(f"  (no {os.path.dirname(OUT)}/ here — the studio repo owns the page)")
    print("ZFL DOC GREEN — the page and the language agree")
    return 0
