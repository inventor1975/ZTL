...
**V. Reznik. Preprint, v2.0.0 — 2026-09-19. Version DOI: minted on upload
...
each a verdict derived from no information. The mark is shown expressible
inside the language and its reach measured: 1840 of 2924 compounds of the
pool are mark-sensitive (`marksens.py`, with the definition beside the
number). And the taint-sink exhibit shows both classical defaults granting
a pass to an unverified sink, with no conservative default available —
whence, *as a decision procedure, ZTL strictly dominates classical logic,
while remaining exactly its equal as a system of proofs on classical
logic's own domain* (`ztl_taut_is_classical`). Every number of this frame
is regenerated by `paper/core_logic_checks.py`, now a stand of the
regression. v2.0.0 is also the structural release: the repository is the
logic alone — the seven-language taint analyzer moved to
`inventor1975/introspect`, the natural-language studio to
`inventor1975/ztlstudio`, each vendoring a copy of this core.
v1.4.1 (2026-09-07) corrected §2: all ten ZTL tables are shown — the
...
stroke and Peirce's arrow are not complete alone; both implications and
both abjunctions are), and the basis ↛ reads as the credit detector; the
...
require the boundary between learning and world change; plus the
derivation theorem (§23) — the rules that hold on marked data transport
truth but cannot mint it.**
...
Its relation to classical logic has two halves, both machine-checked, and
...
exhibited witness, none left open — fourteen theorems about unverified
data (`*_needs_ground` in Lean), where classical logic decides none of the
twenty-six. The mark is expressible inside the language (isZ(x) = ¬(x↔x)),
and on 1840 of 2924 compounds of the pool the verdict depends on whether
an atom is unverified or false — a distinction the usual substitution
"unverified := false" cannot draw at all. That substitution is shown to be
Bochvar's external logic of 1938, cell for cell on every binary
connective; ZTL parts from it in eight cells, each a place where a verdict
is derived from the absence of information. **No default replaces the
mark:** in the taint-sink case both classical defaults grant a pass to an
...
account with the census of the twenty-six classical laws on a marked atom
(twelve hold there, modus ponens among them; fourteen are refuted with a
witness, every one a law of "truth from form"; none is left undecided);
...
algebraic passport (idempotence, refuted on the mark, yields exact truth
...
the derivation theorem (the twelve rules that hold on marked data
transport truth but cannot mint it — from no premises nothing is derivable, even the guarded
...
What is shown without qualification is that the common denominator
carries a full logical development: a calculus, quantifiers, modal and
...
below; it is the negative routes that need verified ground, §3.1.)
...
p→q = ¬p∨q, p⊕q = (p∧¬q)∨(¬p∧q), p↔q = (p∧q)∨(¬p∧¬q) are identities
that hold on the mark as well.
...
definitions of the derived connectives. Refuted there — each in Lean as a
`*_needs_ground` theorem: ¬¬p=p, both De Morgan
laws, contraposition-as-identity, ⊕=¬(↔), idempotence of ∧/∨,
absorption, the units (p∧T=p, p∨F=p), excluded middle, p→p, Peirce's
law, q→(p→q). The common trait of the fourteen: each fails only on Z, and
each is a law of "free truth" (truth from form or from polarity flip).

**Not usable is not missing: all fourteen are refuted with a witness
(MEASURED + Lean).** A count of 12 against 14 reads as a ledger of loss, and that
...
**What the mark buys, in numbers (MEASURED, `marksens.py`).** The mark is
sayable inside the object language: isZ(x) = ¬(x↔x) is T on the mark and
F on both T and F (`isZ_detects`), so the logic can state "this input is
unverified" in a formula whose own value is a verdict. Call a compound
*mark-sensitive* if on some valuation its verdict changes when every mark
in that valuation is replaced by F — the definition must stand beside the
number, because a narrower reading of the same question understates it by
twenty points:

    mark-sensitive (all marks → F changes it)  1840 of 2924 (63 %)
    the mark on ONE atom, the other verified   1263 of 2924 (43 %)
    only with BOTH atoms unverified              10, e.g. (p→p)∨(q→q)
    under "unverified := false"                   0 (Z, F: one input)
...
and hereditary (§19); identity p→p and excluded middle p∨¬p are both
refuted on Z — each takes the value F.
...
Of 14 classical inference rules taken as entailments, 12 hold on marked
data — including contraposition-as-a-rule (p→q ⊨ ¬q→¬p) and the K-rule
(q ⊨ p→q), whose law twins are refuted in §3.1. Two do not: ¬¬-elimination (¬¬p ⊨ p; Z
...
3. **Collapsing Z→F at the atom is the classical fork.** The claim
   ...
   among them. With every law restored the rules/laws split has
   nothing left to split, and the system is Bochvar's isomorph B3□ —
   which §4 identifies, cell for cell on every binary connective, with
   the engineering default "unverified := false".
...
The laws that need ground pay for the algebra. The chain, each link verified by
total enumeration and kernel-checked in Lean (module `ZAlgebra`, empty
axiom list):

1. **Idempotence, refuted on the mark, is a truth detector.** J_T(p) = p∧p takes T
...
4. **Algebraizability.** The same-value detector
   ...
   inherit them.
5. **Yet not self-extensional.** p ⊣⊨ p∧p (idempotence is refuted as
   a law on the mark, alive as interderivability) while ¬(p∧p) ⊭ ¬p (at p=Z):
...
the truth detector J_T(x) = x∧x (idempotence refuted on the mark, §3.6)
repairs the classical polynomial. The *ring* needs verified ground: the
unit law x∧⊤ = x, idempotence and the distributivity of ∧ over ⊕ are all
refuted on Z (only the characteristic-2 law x⊕x = ⊥ holds on the mark — it
is an anchor cell), so no multilinear canonical form exists; the normal-form role
...
|---|---|---|
| complete alone | →, ←, ↛, ↚ | 514 |
| not complete alone | NAND, NOR | 18 (one shared cage) |
...
The four directional connectives are complete alone — both implications (the
...
to forbid). No commutative kernel is complete alone, the two classical
one-operator champions included: Sheffer's stroke and Peirce's arrow stall in
...
lift lies in none of these seven; both classical champions are excluded
...
non-commutative directionality, and (iii) the reading of the basis ↛ as
the credit detector. (Our census further finds the two
...
J_T = p∧p of §3.6, derived here from idempotence refuted on the mark, carries
...
|---|---|
| at the atom (predicate) | Libkin–Peterfreund 2VL ≈ Bochvar's B3□ ≈ the default "unverified := false" (below) |
...
its price is the rewriting laws on marked atoms, which the atomic position
preserves (which is why the optimizers chose it), and its gain is the
third outcome of §10's sink, which the atomic position cannot have. A terminological caution:
...
Hilbert style is closed off (axiom K is refuted on the mark, valid formulas are scarce),
...
* **Identities:** both distributions hold (∀ over ∧, ∃ over ∨); both
  quantifier De Morgan laws need ground (counterexample: a one-element domain
  with P=Z — Z hides under negation).
* **Instantiation asymmetry:** UI holds even as a **law**
  (⊨ ∀yP(y)→P(a) — the universal has earned its truth, spend it
  freely), EG as a law is refuted (⊭ P(a)→∃yP(y) at P(a)=Z); only the EG
  rule holds.
* **The rules/laws split continues on the quantifier floor, and the
  ...
  This is the quantifier twin of ¬¬-elimination, which needs ground. The
  converse rule ∀y¬P ⊨ ¬∃yP holds, as does the quantifier swap ∃x∀yR ⊨ ∀y∃xR.
* **Classical ornaments:** quantified LEM is refuted on a marked instance;
  the "drinker paradox" ∃y(P(y)→∀zP(z)) — a classical validity — is refuted
  in ZTL (in a bar with
...
A by-product: ¬∀yP ⊭ ∃y¬P even as a rule — the second quantifier bridge
that needs ground, symmetric to ¬∃ ⊭ ∀¬ (negation hides Z in both directions).
...
as the second quantifier bridge that needs ground, so the calculus's own metatheory
...
— the bridge `¬∀xP ⊢ ∃x¬P` is refuted with `P(c*) = Z`. Each rule
...
* The fourteen refutations of §3.1 are not negotiable item by item: they
  are consequences of three fixed design forks (¬Z=F; greedy collapse;
  Z↔Z=F); extending any of the fourteen laws to the mark requires flipping
  a fork (see §3.5).
* ZTL is not "stronger" than classical logic in the logician's sense of
  proving more: `ztl_taut_is_classical` shows it proves nothing about
  verified data that classical logic does not. The relation that holds is
...
* The Suszko reduction is not escaped — nothing escapes it. It is not
  ...
  verdict (§4).
* Two grades of claim run through the paper and are kept apart: MEASURED
  (machine enumeration over a named pool) and argued. The dominance of §10
  is measured on the pool and on the sink exhibit; the theorems are the
  Lean objects named beside each claim.
...
cells are turned from postulates into theorems, the twelve laws that hold
on the mark and the fourteen `*_needs_ground` refutations, semantic MP, the greediness theorem, the homelessness of
...
**Part II** (same file): the entailment rule battery (11 hold here — modus
ponens is Part I's semantic MP — + 2 refuted on the mark, `rule_dn_elim_needs_ground`
and `rule_taut_concl_needs_ground`, including the split contraposition-rule vs contraposition-law),
...
identities that hold on the mark (imp_def/xor_def/xnor_def — theorems of the core).
Proven:
...
facts — domain-2 quantifiers (∀=zand, ∃=zor: the UI law holds; EG,
¬∃⊭∀¬, ¬∀⊭∃¬, quantified LEM, the drinker — need ground) and dynamics (liar
...
* *Strict expansion in expressive reach.* The clone is exactly the
  ...
  completeness moves to the directional kernels (§3.8: NAND and NOR each
...
intact (C-extension); a mark refutes exactly the **identity laws** —
idempotence S∪S=S and S∩S=S, self-subtraction S∖S=∅, reflexivity S=S
and S⊆S — the same families that need ground in the logic: sets inherit
the census from the tables.
...
(`sub_marked_false`, `seteq_self_marked`), so the identity laws need ground
exactly as they do in the tables; and on clean sets membership, inclusion and
...
* **The pearl: even the identity function is not certifiably injective
  ...
  requires a fully verified domain; the echo of S ⊆ S needing ground.
...
> A simpler cell: (¬p) → (q→q), where the gap is the law of identity
> itself, refuted on the mark.
...
— a conjunction guard of m−1 marks standing over the law of identity,
refuted on the mark — is sound, invariant under every verification of fewer than
...
The census of §3.1 concerns laws; it extends to *paths*. Take the 12
entailment rules of §3.2 that hold on marked data as the only links, the
2 that do not (¬¬-elimination and tautology-in-conclusion) as a loan
library, and
...
cannot mint even those. The twelve rules are *transport, not creation*:
...
(¬¬-introduction holds); from {¬¬p} the closure under the twelve is {¬¬p}
...
*What is hypothesis and what is conclusion.* That the twelve rules of
...
- **Reflexivity is earned.** `Eq(a,a)` is T for a grounded reference but
  **Z** for a marked one — self-identity is not free; an unverified
  ...
  identity analogue of `¬¬p = p`, which needs ground.
...
and ¬∀xP(x) ⊢ ∃x¬P(x) — the bridge that needs ground — returns `stuck`, not `closed`.
...
swap closes (`run_swap_with_delta2`), and that bridge stays
...
against Tomova's criterion in §4; the accompanying benches; and the
2026-09-18 measurements of the relation to classical logic — the 588 = 588
identity of validity sets, the refutation census of the twenty-six laws,
the engineering default identified as external Bochvar, the taint-sink
exhibit and the naming of dominance — with the `*_needs_ground` rename;
Claude
...
statements, the v1.4 assembly, and the v2.0.0 frame — the relation to
