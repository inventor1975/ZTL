...
- Z = взято в КРЕДИТ — допущение, которое код НЕ проверяет;
- E = судить не на чем;
- S = СОБЛАЗН (Seduction): утверждение ИСТИННО, но во ВРЕД — правда, употреблённая \
во вред (утечка памяти, рост структуры без предела, пропавший инвариант, который \
...
def main() -> int:
    ...
    lines = [f"# credit-ledger — {target.name}", "",
             ...
             f"(S = соблазн/Seduction: истинно, но во вред).",
