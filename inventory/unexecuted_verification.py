...
def main():
    ...
    print("\n  УРОВЕНЬ, КОТОРЫЙ ЭТОТ ПРИБОР УСТАНАВЛИВАЕТ: 2 из 5.")
    print("    1 сущность ссылки — пин существует")
    print("    2 сущность точки входа — названный файл/импорт есть НА ПИНЕ  <- здесь")
    print("    3 исполнимость перепроверки — команда отработала на пине")
    print("    4 верность предмета — она проверяет то, что обещает")
    print("    5 годность оракула — заведомо негодный образец ОТВЕРГНУТ")
    print(f"\n  прошло проверку уровня 2      : {ok}")
    print(f"  ПРОВАЛИЛО проверку уровня 2   : {len(bad)}")
    print(f"  не определилось               : {len(unknown)}")

    if bad:
        print("\nПРОВАЛИВШИЕ УРОВЕНЬ 2 (не «неисполнимые» — так говорить вправе только уровень 3):")
    ...
    print("\nПОТОЛОК, названный вслух. Ноль на уровне 2 НЕ означает")
    print("«пути исполнимы»: команда всё ещё падает от нехватки зависимости,\n"
          "не тех аргументов, отсутствующей фикстуры, чужого окружения — или\n"
          "отрабатывает, проверяя НЕ ТОТ предмет. Слово «неисполнимый»\n"
          "принадлежит уровню 3, а туда этот прибор не дотягивается.")
    return 0
