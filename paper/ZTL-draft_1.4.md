...
**Tables — both registers, all twelve** (generated by the principle;
the anchor cells were postulated at design time and are reproduced by
the principle — `ztl.py`; every cell below is read off the kernel's
own definitions, `znot`/`zand`/`zor`/`zimp`/`zxor`/`zxnor` and
`knot`/`kand`/`kor`/`kimp`/`kxor`/`kxnor`). The greedy register, which
issues verdicts:
...
| F | T | T | T |   | F | T | F | F |   | F | F | T | F |
| Z | T | F | F |   | Z | F | F | F |   | Z | F | F | F |
...
| → | T | F | Z |   | ⊕ | T | F | Z |   | ↔ | T | F | Z |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| T | T | F | Z |   | T | F | T | Z |   | T | T | F | Z |
| F | T | T | T |   | F | T | F | Z |   | F | F | T | Z |
| Z | T | Z | Z |   | Z | Z | Z | Z |   | Z | Z | Z | Z |

The two sets are not mirror images of each other, and neither can be
read off the other by a relabelling: they part in 20 of the 48 cells, and
in every one of them the greedy side answers F where the lazy side
keeps Z — the greedy register never returns the mark above the atoms
(the greediness theorem, §3), the lazy one returns it wherever a
reading of the unverified could still change the answer, and no rule
on the cells recovers one table from the other without the tables
themselves (¬Z is F in one and Z in the other; F ∨ Z is F in one and Z
in the other; Z → Z is F in one and Z in the other). Both are needed and
both are shown, because the paper runs on both (§9): the lazy register
is where grounding and the receipt live, the greedy one is where a
verdict is issued.
