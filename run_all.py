...
STANDS = [
    ...
    ("downstream/sgraph_corpus/artifact/controls.py",
     ["ВСЕ 32 КОНТРОЛЕЙ ПРОШЛИ", "отказывает ТОЙ САМОЙ проверкой"]),
    ("downstream/sgraph_corpus/artifact/reconcile.py",
     ["расхождений с ледгером: 0", "ложных притязаний     : 0",
      "материализовано       : 88", "ОПУЩЕНО при притязании VERIFIED: 0",
      "ЕДИНОГО ПРОЦЕНТА ПО B НЕ ПРИВОДИТЬ"]),
    ("downstream/sgraph_corpus/artifact/cohort_c_crosswalk.py",
     ["УСТАНОВЛЕНО: 7/7 исходных разрядов выстояли материализацию"]),
    ("downstream/sgraph_corpus/triage/q_experiment.py",
     ["схема ЗАМОРОЖЕННАЯ: НЕ ОТВЕЧАЕТ НА ЭТОТ ВОПРОС",
      "ВОЗМОЖНОСТЬ A", "П-40 СМЫКАЕТСЯ С П-10"]),
    ("downstream/sgraph_corpus/triage/unit_registry.py",
     ["ОТПЕЧАТОК РЕЕСТРА", "НИ ОДИН код не выбран под нужды"]),
    ("downstream/sgraph_corpus/triage/reentry_p10_p40.py",
     ["РАЗРЕШЁН, но НЕ тем, чем казалось", "ОСТАЁТСЯ НЕРАЗРЕШЁННЫМ"]),
    ("downstream/sgraph_corpus/triage/successor_freeze.py",
     ["ЗАМОРОЗКА ПРЕЕМНИКА ВОССТАНОВЛЕНА ЦЕЛИКОМ"]),
    ("downstream/sgraph_corpus/triage/enumerate_nonpass.py",
     ["НЕПРОШЕДШИХ ЛИЧНОСТЕЙ: 12", "РАВНЫМИ НЕ СЧИТАТЬ"]),
    ("downstream/sgraph_corpus/triage/triage_register.py",
     ["РАЗРЯД C ПУСТ", "ДВА СЕМЕЙСТВА, А НЕ СЕМЬ НЕЗАВИСИМЫХ ДОВОДОВ", "A — ОСПОРЕН"]),
    ("downstream/sgraph_corpus/artifact/crossref_crosswalk.py",
     ["разряд НЕ зависит от длины окна", "вхождений разобрано здесь        : 39",
      "DEPENDENCY_ASSERTED 1, DESIGNATION 12"]),
    ("downstream/sgraph_corpus/artifact/crossref_detector.py",
     ["СОСТАВ ССЫЛОК СОВПАДАЕТ С ЭТАЛОНОМ ПОИМЁННО",
      "случаев С ЯВНОЙ ССЫЛКОЙ в заякоренном тексте: 22"]),
    ("downstream/sgraph_corpus/artifact/freeze.py",
     ["Прибор СООТВЕТСТВУЕТ заморозке", "состав явных ссылок те же"]),
    ("downstream/sgraph_corpus/artifact/original_representation.py",
     ["СОСТАВ СОВПАДАЕТ С ЭТАЛОНОМ ПОИМЁННО",
      "живой вывод: строк 88, без росписи 0"]),
    ("downstream/sgraph_corpus/artifact/coverage.py",
     ["МЕХАНИЧЕСКИМ ЭТО НЕ ЯВЛЯЕТСЯ"]),
    ("downstream/sgraph_corpus/artifact/profile_conformance.py",
     ["ПРИБОР СООТВЕТСТВУЕТ ЗАМОРОЖЕННОМУ КОНТРАКТУ",
      "НЕ ПОКРЫТО пунктами сверки: 0",
      "имён схемы БЕЗ объявления как добавление: 0"]),
    ("downstream/sgraph_corpus/contamination_replacement.py",
     ["ПУНКТ 4 РАБОТАЕТ", "правило 4 НЕПРИМЕНИМО", "12 -> 11"]),
    ("downstream/sgraph_corpus/ledger_counts.py",
     ["ПОЙМАНА — проверка не пуста", "КОНТРОЛЬ ЦЕЛОСТНОСТИ (всякая ссылка определена): ПРОЙДЕН",
      "ВСЕГО: предложений 88"]),
    ("downstream/sgraph_corpus/disposition_ledger.py",
     ["строк в ледгере: 88", "REPRESENTABLE_V0", "несут И расположение-провал, И помету: 0"]),
    ("downstream/sgraph_corpus/control_ledger.py",
     ["ОБНАРУЖЕН — ледгер работает", "не подпирают вывод: 0"]),
    ("inventory/path_representation.py",
     ["ДОГОВОР ПРЕДСТАВЛЕНИЯ ЗЕЛЁНЫЙ", "и отсутствие"]),
    ("inventory/unexecuted_verification.py",
     ["ПОЛОЖИТЕЛЬНЫЙ КОНТРОЛЬ: ПРОЙДЕН",
      "ПРОВАЛИЛО проверку уровня 2   : 0"]),
    ("inventory/assurance_incident_001.py",
     ["ASSURANCE-INCIDENT-001 ВОСПРОИЗВЕДЁН",
      "Подделка с пересчётом отпечатка ПРОХОДИТ сверку"]),
