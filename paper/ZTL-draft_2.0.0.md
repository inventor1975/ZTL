# ZTL — Zero-Trust Logic — v2.0.0

*v2.0.0 is a structural release: the repository was split into ZTL (this logic),
introspect (the taint analyzer), and ztlstudio (the studio). The preprint text
is unchanged from v1.4.1; a v2.0.0 version DOI will be minted on upload. Working
draft — refine here, then build the PDF with `paper/mkpreprint.py`.*

**V. Reznik. Preprint, v1.4.1 — 2026-09-07. Version DOI:
[10.5281/zenodo.22644261](https://doi.org/10.5281/zenodo.22644261). Concept DOI:
...
axiom tier of the four quantifier rules measured — three sound on the
empty axiom list, the fourth only classically and kept outside the
...
(§27). (The ordinals below — "the sixth", "the fifth" — are the order in
which the embeddings landed, not the order of the list above.) Taint tracking is the sixth (`ZFlow.lean`, empty axiom list): Denning's
...
**Tables — all ten of ZTL** (generated by the principle; the anchor
cells were postulated at design time and are reproduced by the principle
— `ztl.py`; the same ten are kept in `ZTL-TABLES.txt`, regenerated and
checked by `ztl_tables.py`, and the six primitive ones are read off the
kernel's `znot`/`zand`/`zor`/`zimp`/`zxor`/`zxnor`). The five primitives
and ↔:
...
| ⊕ | T | F | Z |   | ↔ | T | F | Z |   | ¬(a∧b) | T | F | Z |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| T | F | T | F |   | T | T | F | F |   | T | F | T | T |
| F | T | F | F |   | F | F | T | F |   | F | T | T | T |
| Z | F | F | F |   | Z | F | F | F |   | Z | T | T | T |

| ¬(a∨b) | T | F | Z |   | ¬(a⊕b) | T | F | Z |   | ¬(a→b) | T | F | Z |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| T | F | F | F |   | T | T | F | T |   | T | F | T | T |
| F | F | T | T |   | F | F | T | T |   | F | F | F | F |
| Z | F | T | T |   | Z | T | T | T |   | Z | F | T | T |

**Why the second five are on the page.** Classically five of these ten
are redundant: ↔ is ¬⊕, the two De Morgan laws give the negated
conjunction and disjunction from ¬ and the other connective, and
¬(a→b) is a∧¬b — a reader reconstructs them and no textbook prints
them. Here every one of those routes fails, and MEASURED at the marked
cells: a↔b differs from ¬(a⊕b) in 5 of 9 cells, ¬(a∧b) from ¬a∨¬b in 3,
¬(a∨b) from ¬a∧¬b in 3, ¬(a→b) from a∧¬b in 3 — always where an input
is Z. What does hold is trivial and is the reason: ¬ acts on a verdict,
and a compound's verdict is already classical, so ¬(a∧b) is exactly the
T/F swap of the ∧ table; the negation of a compound never sees the mark
its inputs carried, which is what De Morgan needs. So the second five
carry information of their own and cannot be read off the first five.
(The positive definitions survive — a→b = ¬a∨b, a⊕b = (a∧¬b)∨(¬a∧b),
a↔b = (a∧b)∨(¬a∧¬b), zero cells apart — which is the generating basis
below; it is the negative routes that fall, §3.1.)

**The lazy register** (strong Kleene, §9), in which the mark flows — the
same primitives as the kernel's `knot`/`kand`/`kor`/`kimp`/`kxor`/`kxnor`:

| x | ¬x |   | ∧ | T | F | Z |   | ∨ | T | F | Z |   | → | T | F | Z |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| T | F  |   | T | T | F | Z |   | T | T | T | T |   | T | T | F | Z |
| F | T  |   | F | F | F | F |   | F | T | F | Z |   | F | T | T | T |
| Z | Z  |   | Z | Z | F | Z |   | Z | T | Z | Z |   | Z | T | Z | Z |
...
The two registers are not mirror images of each other either: on the six
primitives they part in 20 of the 48 cells, and in every one of them the
greedy side answers F where the lazy side keeps Z — the greedy register
never returns the mark above the atoms (the greediness theorem, §3), the
lazy one returns it wherever a reading of the unverified could still
change the answer. Both are needed (§9): the lazy register is where
grounding and the receipt live, the greedy one is where a verdict is
issued.
...
Machine verification (`lean/ZClone.lean`, empty
...
holds even where ⊨ p→p fails. The price of that gap is named in §3.2:
...
matrices").** The {¬,∧,∨} core of ZTL fits the templates
...
function** (E55 — E-numbers name the expeditions, the numbered entries
of the repository's log — `ZCutElim.lean`, empty axiom list): derivations are an
...
**Quantifier tableaux** (MEASURED + Lean): the finite-domain rules continue
...
**Arbitrary domains: parameter tableaux (MEASURED + Lean).** Over arbitrary
...
honestly split: **soundness is proved for the rules and the search in
the Lean port (§27; the classical rule F:∀ only outside the corpus) and
measured for this engine** — every sequent the engine proves is re-checked by total enumeration over finite domains, every
...
domain. The first is proved: `stuck_refutes` — for closed Γ and φ, a
...
* ZTL owns no physical clock: duration, tense and "how long" are
  outside its axis; the only clock is the arrival of ground (§21).
...
**Part II** (same file): the entailment rule battery (11 alive here — modus
ponens is Part I's semantic MP — + 2 fallen, including the split contraposition-rule vs contraposition-law),
...
**Conclusion (the architectural result of this stage):** the
...
* *Strict expansion in expressive reach.* The clone is exactly the
  ...
  only the four directional kernels →, ←, ↛, ↚ survive, with ↛ read as
  the credit detector).
...
it (in a private design note, staked and deliberately unimplemented):
...
**The parity result — total (MEASURED).** All cycles of length 1–5, all inversion
...
**The fence depth is exactly m−1 (MEASURED + Lean).** How deep must a
...
reachable at depth m−1, it is reachable nowhere else. One step was
left as prose at E33 rather than smuggled into the formalisation — that
a check inspecting fewer than m−1 atoms must leave some guard unverified
is finite counting, not logic — and E58 later proved it
(`length_ge_of_all_below`, below). Hence
...
* **The linearity hypothesis is load-bearing, not decorative**, and
  ...
  can move. All of the residual inexactness (7% — 994 of the 14,530 pending
  cells above, `lab/label/bench.py`) sits in cells where a marked atom occurs more than once — the same
...
* **The unification result** (a theorem for interval constraints,
  `ZCombine.lean`; measured beyond them): verify of §19 is a special case of
...
**The sixth twin is now an embedding; this section's own claims are
...
with `zcombine` first and are kernel-checked below (`ZCombine.lean`, E39); the tag stays MEASURED + Lean for what the closing
...
measurement. The positive half is no longer instance-bound:
`ZEqGeneric.lean` (§25) proves it for an arbitrary type with decidable
equality; what stays exhaustive is the failures, which need only a
countermodel. (Leibniz here is a one-line `congrArg`: once an equality is
...
layer: a fifth disposition (the judge's dispositions are the kinds of work
order it can issue) for "this ground has no subject", a rule that
...
a fragment-embedding theorem for the remaining three traditions of §1
(as originally listed) — ALL SIX are now done, and the entry stays as the record of what it
...
from inside, §3.4, under IEEE's name; and `!=` proved to be `SignN` of the
...
and GROUP BY run on) — ZTL has one equality and it withholds (§12's
