...
## 5. Исчисление: постановка задачи (следующий этап)

Ограничения, установленные измерениями: гильбертовский стиль закрыт
(аксиома K пала; валидных формул мало), теорема дедукции односторонняя,
правила почти в полном классическом комплекте. Следствие: исчисление
строится секвенциями/таблó. Кандидат-архитектура — **знаковые таблó**
(Rousseau, Hähnle) для конечнозначных логик: узлы вида «S:φ», S ∈
{T,F,Z}, ветвление по таблицам; корректность и полнота для конечных
таблиц получаются конструктивно и сверяются перебором против ⊨.
Отдельный вопрос: какова роль знака Z:φ при теореме жадности (Z возможен
только на атомах) — ожидается вырожденная и потому простая ветка.
...
Знаковые таблó + машинная сверка (§5) → кванторы на конечных доменах →
Lean-порт стабилизированных утверждений → формализация карантинного
флага как конструкции неподвижной точки (à la Крипке) над таблицами ZTL.
