...
\begin{theorem}[Parameter tableaux, arbitrary domains]\label{thm:fo}
  ...
  arbitrary domains (fresh witnesses exactly where the weak signs
  live); soundness measured ($13/13$ verdicts cross-checked, open
  branches yield verified countermodels), completeness by Hintikka
  saturation (argued); undecidability via the J-guard embedding of
  classical FO. Lean port on the roadmap.
\end{theorem}

\begin{theorem}[Quantifier tableaux, finite domains]\label{thm:quant}
  \uses{thm:closesiff, def:jops}\lean{V.cover_allF_T, V.cover_allF_F, V.cover_exF_T, V.cover_exF_F, V.ui_mem, V.eg_mem, V.tab_ui, V.tab_drinker_fails}\leanok
...
\begin{theorem}[Expedition twins]\label{thm:exped}
  ...
  marked pair collapses the injectivity certificate; a nondegenerate
  mark does not equal itself; atom verdicts are the modal thresholds.
\end{theorem}
