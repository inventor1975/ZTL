...
""" + VALUE_MARKS + FRAME_EDGE + REGISTER + """
...
оборачивание/рамка, И ОБЯЗАТЕЛЬНО  отрезано: <что вынесено за край>). В конце — раздел ВЫБОР: какие ходы ОСТАВить (T, и T? с явным \
...
def assemble(verdicts_dir: pathlib.Path, out: pathlib.Path, source_name: str = "") -> pathlib.Path:
    ...
    развилки = []
    _FORK_RE = re.compile(r"\[(?:S\s*\|\s*T\?|T\?\s*\|\s*S)[^\]]*\]")
    ...
    head = [f"# credit-ledger — {source_name or verdicts_dir.name}", "",
            ...
            + (f".  Развилок по жанру: {len(развилки)}." if развилки else "."),
            ""]
    if развилки:
        head += [f"**ЖАНР НЕ РЕШИЛ у {len(развилки)} ход(а/ов)** — судья объявил "
                 "развилку вместо догадки, как и требует рубрика. Метку закрывает "
                 "ЧЕЛОВЕК, объявив жанр (`prepare --genre …`) и пересудив кусок.", ""]
