# -*- coding: utf-8 -*-
"""
zsew_attack — the external reviewer's adversarial cases against the E29 seam, run before
any fix, so the failure surface is measured and on the record.

the downstream reviewer reviewed the seam sketch (2026-07-21) and named its central
...
expected rejection from lack of representational capacity (the external reviewer).
...
def run():
    print("=" * 78)
    print("THE EXTERNAL REVIEWER'S CASES vs THE E29 SEAM — measured before any fix")
    print("=" * 78)
    ...
    print("\n  The decisive #1, in the external reviewer's own five distinctions:")
    ...
    print()
    print("  ROOT CAUSE (the external reviewer): the seam must join TYPED CLAIM")
