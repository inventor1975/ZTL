...
(\texttt{lean/ZTL.lean}, \texttt{lean/QuantumWitness.lean}, \texttt{lean/Contextuality.lean})\\
\small Verify from zero: \S\ref{sec:mirror} --- three files, three commands, no dependencies}
...
quantum contextuality (Mermin--Peres, GHZ) --- and record them as three
machine-checked components in Lean~4 on the \emph{empty axiom list} (no
...
is not compositional is the truth VALUE, not the lattice. The falls saturate
...
Author's & ZTL, the zero-trust paracomplete logic~\cite{ztl}; the arrangement of the four as resolutions of one diagonal; the three Lean witnesses on the empty axiom list (the two poles + the contextuality core); the measured ladder of falls and its saturation & synthesis + machine-checked \\
