...
STANDS = [
    ...
    ("inventory/sgraph_corpus/artifact/controls.py",
     ["ВСЕ 23 КОНТРОЛЕЙ ПРОШЛИ", "отказывает ТОЙ САМОЙ проверкой"]),
    ("inventory/sgraph_corpus/artifact/reconcile.py",
     ["расхождений с ледгером: 0", "ложных притязаний     : 0",
      "ОБЛАСТЬ ПРОГОНА СУЖЕНА ПО УКАЗАНИЮ: 3 из 88"]),
    ("inventory/sgraph_corpus/artifact/original_representation.py",
     ["СОСТАВ СОВПАДАЕТ С МАНИФЕСТОМ ПОИМЁННО",
      "предложений БЕЗ росписи (личность не выводится): 0"]),
