...
def measure(d: dict, opens: str = "effective_at") -> dict:
    ...
    return {
        ...
        # Conditions 7 and 8, as amended under ADMISSIBILITY-CONSEQUENCE
        # BOUNDED IMPLEMENTATION WARRANT 001 (Arkadiy Miteiko, 2026-08-29).
        #
        # WAS: "admissibility_consequence": None, plus a prose note.
        # A field PRESENT with the value null is still CARRIED by the record,
        # and null is ambiguous four ways at once — not computed, not
        # applicable, false, absent. The warrant prohibits both null and a
        # manufactured consequence used to encode a refusal.
        #
        # The admissibility question here ARISES (these acts may or may not be
        # relied upon) and CANNOT PRESENTLY BE ANSWERED (no institutional
        # authority is supplied to this layer). That is the warrant's case B.
        # So the consequence field is ABSENT — the schema declares it
        # conditional, which is what makes the absence readable rather than
        # indistinguishable from loss — and the refusal is carried in its own
        # typed field, OUTSIDE the target-plane result, as the warrant requires.
        "admissibility_determination": {
            "state": "REFUSED_NO_WARRANT",
            # (1) the question arises
            "question_arises": True,
            # (2) why it cannot be answered now
            "reason": (
                "whether an unresolved act may be relied upon for a given use is "
                "an institutional rule; this layer classifies evidence and has no "
                "authority to invent one"
            ),
            # (3) what would make an answer possible
            "completion_condition": (
                "a separately identified institutional authority supplies a rule "
                "binding this epistemic classification to a reliance consequence, "
                "attributable to that authority and scoped to the use in question"
            ),
            "warrant_ref": "ADMISSIBILITY-CONSEQUENCE-BOUNDED-WARRANT-001",
        },
    }
